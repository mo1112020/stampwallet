...
| "Prioritize the roadmap" | Product: backlog scoring and next-up recommendation |
...
| "Audit our security" | DevOps/Security: infra + dependency audit |
...
- `/ai-ceo:product:prioritize` -- Score and re-order the backlog
- `/ai-ceo:product:spec "feature"` -- Write a handoff-ready PRD
- `/ai-ceo:product:pricing-review` -- Review plan limits/pricing against goals
...
- `/ai-ceo:devops:audit` -- Infra/dependency/secrets security audit
- `/ai-ceo:devops:incident "description"` -- Incident response and post-mortem
- `/ai-ceo:devops:deploy-check` -- Pre-production deploy verification
...
## StampWallet Codebase Instructions

This repo (`stampwallet`, the primary WalletOS product) has its own coding conventions that apply whenever the Dev department or any sub-agent touches code here -- read them before writing code:
