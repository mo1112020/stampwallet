...
export const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "rounded-2xl border border-[var(--line)] bg-[var(--surface)] shadow-[0_1px_2px_hsl(var(--shadow-color)/0.06)]",
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <p ref={ref} className={cn("text-[13px] text-[var(--muted)]", className)} {...props} />
));
